# -*- coding: utf-8 -*-
"""LanceDB vector store for semantic memory search.

Uses sklearn TfidfVectorizer for lightweight, zero-dependency embeddings
(no model download needed). Falls back to random projections if sklearn unavailable.
"""

from __future__ import annotations

import pickle
from pathlib import Path

import lancedb
import numpy as np

DB_DIR = Path(get_config().data_dir) / "lancedb_data"
VEC_CACHE_PATH = DB_DIR / "vectorizer.pkl"
EMBEDDING_DIM = 256

# ── Embedding backend ───────────────────────────────────────────────────

_vec_cache: dict = {}


def _get_vectorizer():
    """Get or create a TfidfVectorizer for text embedding."""
    if "vectorizer" in _vec_cache:
        return _vec_cache["vectorizer"]

    try:
        from sklearn.feature_extraction.text import TfidfVectorizer

        if VEC_CACHE_PATH.exists():
            with open(VEC_CACHE_PATH, "rb") as f:
                vec = pickle.load(f)
        else:
            vec = TfidfVectorizer(
                max_features=EMBEDDING_DIM,
                analyzer="char_wb",
                ngram_range=(2, 4),
                lowercase=True,
            )
            # Fit on a minimal corpus so it can transform immediately
            vec.fit([
                "architecture decision api contract database schema frontend backend bug task deployment coding rule dependency integration",
                "create read update delete search memory project candidate review compact prune deprecated",
                "import export config environment variable secret key token authentication authorization",
            ])

        _vec_cache["vectorizer"] = vec
        return vec
    except ImportError:
        return None


def _save_vectorizer() -> None:
    """Persist the vectorizer state."""
    vec = _vec_cache.get("vectorizer")
    if vec is not None:
        VEC_CACHE_PATH.parent.mkdir(parents=True, exist_ok=True)
        with open(VEC_CACHE_PATH, "wb") as f:
            pickle.dump(vec, f)


def _embed_sklearn(texts: list[str]) -> np.ndarray:
    """Embed texts using sklearn TfidfVectorizer."""
    vec = _get_vectorizer()
    if vec is None:
        return _embed_random(texts)

    # Refit if vocabulary is small and we have new text
    try:
        result = vec.transform(texts).toarray().astype(np.float32)
    except Exception:
        # Partial fit if needed
        vec.fit(texts)
        result = vec.transform(texts).toarray().astype(np.float32)
        _save_vectorizer()

    # Normalize rows
    norms = np.linalg.norm(result, axis=1, keepdims=True)
    norms[norms == 0] = 1.0
    result = result / norms
    return result


def _embed_random(texts: list[str]) -> np.ndarray:
    """Fallback: random projection embeddings (deterministic per text)."""
    import hashlib

    dim = EMBEDDING_DIM
    result = np.zeros((len(texts), dim), dtype=np.float32)
    for i, text in enumerate(texts):
        # Deterministic hash-based pseudo-random embedding
        seed = int(hashlib.md5(text.encode("utf-8")).hexdigest()[:8], 16)
        rng = np.random.RandomState(seed)
        vec = rng.randn(dim).astype(np.float32)
        result[i] = vec / (np.linalg.norm(vec) + 1e-8)
    return result


def embed_texts(texts: list[str]) -> np.ndarray:
    """Embed a list of text strings into normalized vectors."""
    return _embed_sklearn(texts)


def embed_query(query: str) -> np.ndarray:
    """Embed a single query string."""
    return embed_texts([query])[0]


def cosine_similarity(a: np.ndarray, b: np.ndarray) -> float:
    """Cosine similarity between two vectors."""
    return float(np.dot(a, b))


# ── LanceDB Store ───────────────────────────────────────────────────────


class LanceDBStore:
    def __init__(self, db_path: str | None = None) -> None:
        self.db_path = db_path or str(DB_DIR)
        self.db_path_obj = Path(self.db_path)
        self.db_path_obj.mkdir(parents=True, exist_ok=True)
        self.db = lancedb.connect(self.db_path)

        # Ensure table exists
        if "memories" not in self.db.table_names():
            # Create with a dummy row so schema is established
            dummy_vec = embed_texts(["initialize"])[0].tolist()
            self.db.create_table(
                "memories",
                [{"id": "init", "project_id": "init", "vector": dummy_vec, "text": "init"}],
            )
            tbl = self.db.open_table("memories")
            tbl.delete("id = 'init'")

    def _get_table(self):
        return self.db.open_table("memories")

    def insert(self, memory_id: str, project_id: str, text: str) -> None:
        """Insert a memory embedding."""
        vec = embed_texts([text])[0].tolist()
        tbl = self._get_table()
        tbl.add([{
            "id": memory_id,
            "project_id": project_id,
            "vector": vec,
            "text": text[:1024],
        }])

    def insert_batch(self, items: list[dict]) -> None:
        """Batch insert memory embeddings. Each item: {id, project_id, text}."""
        if not items:
            return
        texts = [item["text"] for item in items]
        vecs = embed_texts(texts)
        rows = [
            {"id": item["id"], "project_id": item["project_id"],
             "vector": vec.tolist(), "text": item["text"][:1024]}
            for item, vec in zip(items, vecs)
        ]
        tbl = self._get_table()
        tbl.add(rows)

    def search(
        self, query: str, project_id: str | None = None, limit: int = 10,
    ) -> list[dict]:
        """Semantic search for similar memories."""
        query_vec = embed_texts([query])[0].tolist()
        tbl = self._get_table()

        if project_id:
            results = tbl.search(query_vec).limit(limit * 3).to_list()
            results = [r for r in results if r.get("project_id") == project_id][:limit]
        else:
            results = tbl.search(query_vec).limit(limit).to_list()

        # Calculate cosine similarity
        qv = np.array(query_vec, dtype=np.float32)
        for r in results:
            if "vector" in r:
                r["similarity"] = cosine_similarity(qv, np.array(r["vector"], dtype=np.float32))
            else:
                r["similarity"] = 0.0
            r.pop("vector", None)

        # Persist vectorizer after search (vocabulary may grow)
        _save_vectorizer()

        return sorted(results, key=lambda r: r.get("similarity", 0), reverse=True)

    def search_similar(
        self, query_text: str, project_id: str, threshold: float = 0.85, limit: int = 5,
    ) -> list[dict]:
        """Find similar memories above a similarity threshold."""
        results = self.search(query_text, project_id=project_id, limit=limit)
        return [r for r in results if r.get("similarity", 0) >= threshold]

    def delete(self, memory_id: str) -> None:
        """Remove a memory embedding."""
        tbl = self._get_table()
        tbl.delete(f"id = '{memory_id}'")

    def delete_by_project(self, project_id: str) -> None:
        """Remove all embeddings for a project."""
        tbl = self._get_table()
        tbl.delete(f"project_id = '{project_id}'")

    def update(self, memory_id: str, project_id: str, text: str) -> None:
        """Update a memory embedding (delete + re-insert)."""
        self.delete(memory_id)
        self.insert(memory_id, project_id, text)

    def count(self) -> int:
        return self._get_table().count_rows()
